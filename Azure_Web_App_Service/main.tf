...
resource "azurerm_windows_web_app" "webapp" {
  ...
  logs {
    ...
    http_logs {
      azure_blob_storage {
        retention_in_days = 7
        sas_url           = "https://${azurerm_storage_account.appstorewl8103.name}.blob.core.windows.net/${azurerm_storage_container.weblogswl.name}${data.azurerm_storage_account_blob_container_sas.accountsas.sas}"
      }
    }
  }
